...
- `GET /api/v1/stocks/realtime?codes={code1},{code2}`
- `GET /api/v1/stocks/{code}/realtime`
- `GET /api/v1/stocks/{code}/intraday?trade_date={trade_date}&interval={interval}`
...
`/stocks/{code}/daily` 返回的 OHLC、量均线、MA、MACD、BOLL、KDJ、RSI、CCI、WR、ATR 与筹码字段用于绘制日线蜡烛图、多副图和联动行情快照。筹码图直接使用同一交易日 `chip.chart.x/y`。

最新行情遵循以下契约：

- 指数、个股批量和个股单只 realtime 接口是价格快照，核心字段为 `price` 与 `source_time`，不等同于分钟 K
- 当日分钟 K 只使用 intraday 接口返回的真实 `timestamp/open/high/low/close` 序列；`interval` 可取 `1m`、`5m`、`15m`、`30m`、`60m`、`120m`
- 页面可见且市场活跃时，指数与个股价格快照每 5 秒刷新，当日分钟 K 每 30 秒刷新；后端状态为 `closed` 后停止自动轮询，并保留最近一次成功行情
- 请求失败或数据变旧时保留已有值并标记延迟；前端不把快照价格复制为 OHLC、不伪造时间戳、不累计或合成分钟历史

前端也不自行推导指标、不接入第三方行情、不补造样例数据，不生成单股研判、推荐或交易价位。顶部大盘指数在闭市后仍保留最近一次成功的行情，并标注“已闭市”和“最后行情”。`Stock_Project` 后端可在其项目目录启动：

```bash
uvicorn app.api.app:create_app --factory --host 0.0.0.0 --port 8100
```

`npm run dev` 只启动前端；接口不可用时，界面会展示明确的失败或空状态，不会伪造行情。
...
启动 `npm run dev` 后，可通过 `http://localhost:5188/backend-api/api/v1/...` 检查真实开发代理链路；请求应始终经过 `/backend-api`，而不是从浏览器直接访问后端主机。
